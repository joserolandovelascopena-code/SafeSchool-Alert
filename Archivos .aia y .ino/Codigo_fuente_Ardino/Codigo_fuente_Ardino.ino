...
void enviarEvento(String codigoEvento) {
  Serial.println(codigoEvento);
}
...
void sensorHumo() {
  ...
  if (humo >= UMBRAL_SEGURO && esEmerg == SEGURO) {
    enviarEvento("Incendio,Edif_A,Activa");
    esEmerg = EMERGENCIA_ACTIVA;
    tipoEmerg = INCENDIO;
    mensajeText("Emerg: Incendio", "Edificio A: 1C");
  } else if (humo < UMBRAL_SEGURO && tipoEmerg == INCENDIO && esEmerg == EMERGENCIA_ACTIVA) {
...
void btnsEmergencias() {
  ...
  if (estadoBtnMe1 == HIGH && esEmerg == SEGURO) {
    enviarEvento("Medica,Edif_A,Activa");
    esEmerg = EMERGENCIA_ACTIVA;
    tipoEmerg = MEDICA;
    zona = EDIF_A;
    actualizarLCD();
  ...
  if (estadoBtnSe1 == HIGH && esEmerg == SEGURO) {
    enviarEvento("Seguridad,Edif_A,Activa");
    esEmerg = EMERGENCIA_ACTIVA;
    tipoEmerg = SEGURIDAD;
    zona = EDIF_A;
    actualizarLCD();
  ...
  if (estadoBtnMe2 == HIGH && esEmerg == SEGURO) {
    enviarEvento("Medica,Edif_B,Activa");
    esEmerg = EMERGENCIA_ACTIVA;
    tipoEmerg = MEDICA;
    zona = EDIF_B;
    actualizarLCD();
  ...
  if (estadoBtnSe2 == HIGH && esEmerg == SEGURO) {
    enviarEvento("Seguridad,Edif_B,Activa");
    esEmerg = EMERGENCIA_ACTIVA;
    tipoEmerg = SEGURIDAD;
    zona = EDIF_B;
    actualizarLCD();
  ...
  if (estadoBtnMe3 == HIGH && esEmerg == SEGURO) {
    enviarEvento("Medica,Edif_C,Activa");
    esEmerg = EMERGENCIA_ACTIVA;
    tipoEmerg = MEDICA;
    zona = EDIF_C;
    actualizarLCD();
  ...
  if (estadoBtnSe3 == HIGH && esEmerg == SEGURO) {
    enviarEvento("Seguridad,Edif_C,Activa");
    esEmerg = EMERGENCIA_ACTIVA;
    tipoEmerg = SEGURIDAD;
    zona = EDIF_C;
    actualizarLCD();
...
void descifrarEstados(String comando) {
  if (esEmerg != SEGURO) {
    ...
    if (comando == "Desc,All") {
      ...
      // Limpiar componentes físicos inmediatamente
      noTone(buzzer);
      ...
      Serial.println(F("[OK] Emergencia desactivada. Sistema en estado SEGURO."));
      return;
    }
...
void sirena() {
  ...
  switch (esEmerg) {
    case EMERGENCIA_ACTIVA:
      sirenaContinuaAsync(400, 600, 100);
      break;

    case MEDICA_PROCESO:
      sirenaDiscontinuaAsync(80, 600, 400);
      break;
...
void estadoSeguroSystem() {
  ...
  if (yaCambioASeguro && millis() - ultimoCambioSeguro >= 1500) {
